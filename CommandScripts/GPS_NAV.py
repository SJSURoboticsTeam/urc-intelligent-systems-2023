...
class VFH_obstacle_avoidance:
    ...
    def get_target_angle(
        ...
        smoothed_histogram = np.convolve(
            histogram, np.ones(3), mode="same"
        )  # Smooth histogram
        # print(f"{smoothed_histogram=}")
        target_direction = np.argmax(
            smoothed_histogram
        ...
        angle_difference = (
            rover_angle - lidar_angle_start
        ) % 360  # Adjust angle difference to be within 0-360
        rotation_angle = (
            target_angle - angle_difference
        ) % 360  # Adjust rotation angle to be within 0-360
        ...
        return lidar_data.index(
            max(lidar_data[rotation_angle : rotation_angle + sector_size + 1])
        )
...
class GPS_Nav:
    def __init__(self, max_speed, max_steering, GPS, compass, GPS_coordinate_map):
        ...
        self.steer_controller.proportional_on_measurement = (
            True  # Use derivative of error instead of error for Kp
        )
    ...
    def PID_steer(self, commands, steer_output, angle):
        speed_error = self.max_steering / abs(
            steer_output
        )  # scale speed error based on steering output
    ...
    def get_steering(self, current_GPS, GPS_target):
        ...
        distance = round(
            self.AutoHelp.get_distance(current_GPS, GPS_target)[0] * 1000, 3
        )
        ...
        if distance <= 3:
            ...
            ret = self.stop_rover(self.commands)
            ret["LS"] = 3
            return ret
