...
def exploration_step(obstacles: list[LineString]):
    ...
    nearest_node = min(range(len(node_vals)), key=lambda i: polar_dis(node_vals[i], rpoint))
    if any([intersects(LineString((polar_to_cart(node_vals[nearest_node]),polar_to_cart(rpoint))), obs) for obs in obstacles()]):
        return
...
class RRT_Navigator(Navigator):
    def __init__(self, worldview) -> None:
        ...
        self.goal = None
        self._service = Service(grow_tree, "RRT Service", lambda: [LineString(obs) for obs in worldview.get_obstacles])
