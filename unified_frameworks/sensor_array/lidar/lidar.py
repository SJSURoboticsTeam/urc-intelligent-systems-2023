...
from sensor_array.lidar.actual_lidar import ActualLidar
from sensor_array.lidar.bridge_lidar import BridgeLidar
import importlib
from sensor_array.lidar.fake_lidar import FakeLidar
...
class Lidar(_Abstract_Service):
    def __init__(self, preference=config["lidar_preference"]) -> None:
        ...
        if lidar is None:
            raise NoLidarException(
                f"Failed to connect on the following lidars {preference}"
            )
        self._lidar: _Lidar = lidar
        self._lidar.disconnect()
        if config["verbose"]:
            print(f"Using {type(self._lidar)}")
    ...
    def stop_service(self):
        ...
        if config["verbose"]:
            print("Disconnecting from Lidar")
        self._lidar.disconnect()
    ...
    def get_point_clouds(self):
        ...
        """
        return [
            [(radians(deg), mm / 1000) for q, deg, mm in self._lidar.get_measures()]
        ]
    ...
    def get_obstacles(self, thresh=1):
        ...
        )  # use [].extend(measurement for clarity)
